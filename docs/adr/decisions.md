...
**Decyzja:** nowa tabela `user_profile`, jedna na usera (nie per-persona — dane biometryczne są wspólne niezależnie od tego, z którą personą user rozmawia; odróżnić od `personas.persona_constraints`, które jest specyficzne dla danej persony). Wypełniana przez nowe narzędzie `update_user_profile` (ta sama klasa co `log_result` — walidacja, częściowa aktualizacja, błąd wraca do modelu jako tool response), dostępne dla każdej persony. `ContextBuilder` wykrywa niekompletny profil i dokleja dynamiczną instrukcję "dopytaj naturalnie o brakujące dane" do promptu — znika automatycznie, gdy profil się uzupełni. Formularz `/profile` jako fallback dla userów wolących wypełnić dane wprost.
