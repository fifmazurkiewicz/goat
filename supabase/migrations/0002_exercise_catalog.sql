-- Exercise catalog — ADR-14. Reference content (like persona_templates/plan_templates),
-- seeded via migration, without its own domain service — thin router + repository.
-- See docs/technical/database-schema.md, docs/technical/frontend.md (/settings).

create table public.exercises (
  id uuid primary key default gen_random_uuid(),
  slug text not null unique,
  name text not null,
  persona_type text not null,          -- personas.type for which the exercise is visible in /settings
  level text not null,                 -- 'beginner' | 'intermediate' | 'advanced'
  categories text[] not null default '{}',
  short_description text not null,
  detail_full text not null,           -- "Execution"
  common_mistakes text not null,       -- "Common mistakes"
  photo_path text,                     -- path in Supabase Storage (public bucket 'exercise-photos'); NULL = no photo
...
-- No INSERT/UPDATE/DELETE for `authenticated` — content managed only via migrations/seed,
-- like persona_templates/plan_templates. Adding editability is a separate, future decision.

-- ============================================================
-- SEED — starter set (extend with subsequent migrations as the catalog grows)
-- ============================================================
...
values
  ('przysiad-ze-sztanga', 'Przysiad ze sztangą', 'motor_coach', 'intermediate',
   ...
   || 'kolana idą w linii stóp, aż uda będą równolegle do ziemi lub niżej. Wstawanie przez '
