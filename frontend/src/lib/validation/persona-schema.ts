...
export const personaFormSchema = z
  .object({
    base_template_id: z.string().min(1, "Wybierz szablon"),
    persona_type: z.string().min(1),
    name: z.string().trim().min(2, "Nazwa musi mieć min. 2 znaki").max(60, "Maks. 60 znaków"),
    system_prompt: z.string().trim().min(20, "Opisz personę dokładniej (min. 20 znaków)").max(4000, "Maks. 4000 znaków"),
    persona_constraints: z.string().trim().max(500, "Maks. 500 znaków").optional().default(""),
    detail_level: z.enum(["simple", "detailed"]),
    plan_template_id: z.string().nullable().default(null),
    columns: z.array(personaColumnSchema).min(1, "Dodaj przynajmniej jedną kolumnę").max(8, "Maksymalnie 8 kolumn"),
